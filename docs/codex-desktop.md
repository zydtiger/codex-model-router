...
1. Generate the combined catalog using the steps in the README.
2. Start the router and verify it with `healthcheck --config <router-config>`.
...
absolute catalog path. For this native-authentication setup, remove any root
...
```sh
codex -c 'model_catalog_json="/absolute/path/to/model-catalog.json"' debug models
```
...
Use `validate --config <router-config>` to inspect exact model routing. The upstream
